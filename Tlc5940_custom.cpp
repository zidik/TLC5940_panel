...
/** Packed grayscale data, 24 bytes (16 * 12 bits) per TLC.
	...
	MODIFIED: Now there is DATABUFFERS(default=15) of them
		  */
	
uint8_t tlc_GSData[DATABUFFERS][NUM_TLCS * 24];
...
uint8_t Tlc5940::update(void)
{
    ...
    uint8_t *p = tlc_GSData[activeBuffer];
    while (p < tlc_GSData[activeBuffer] + NUM_TLCS * 24) {
        tlc_shift8(*p++);
        tlc_shift8(*p++);
        tlc_shift8(*p++);
    }
...
void Tlc5940::set(TLC_CHANNEL_TYPE channel, uint16_t value)
{
    ...
    uint8_t *index12p = tlc_GSData[activeBuffer] + ((((uint16_t)index8) * 3) >> 1);
...
void Tlc5940::setActiveBuffer(uint8_t buffno)
{
	if (buffno >= DATABUFFERS) return;
	activeBuffer = buffno;
}
